...
public class KdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    {
        return _findClosest(position, null, nearestDist);
    }
    ...
    public IEnumerable<T> FindClose(Vector3 position, float nearestDist = float.MaxValue)
    {
    ...
    protected T _findClosest(Vector3 position, List<T> traversed = null, float nearestDist = float.MaxValue)
    {
        if (_root == null)
        {
            return null;
